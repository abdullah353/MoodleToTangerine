<?php 
 $enrolquery = mysqli_query($con,'SELECT id FROM '.$CFG->prefix.'enrol WHERE roleid = 5 AND courseid = 3');
 ...
 $userquery = mysqli_query($con,'SELECT firstname, lastname FROM `'.$CFG->prefix.'user` WHERE id = '.$userEnrollfetch['userid']);
      $usergenerator = mysqli_fetch_array ($userquery);      
		$usergenerated .= '[ "'.$usergenerator['firstname'].' '.$usergenerator['lastname'].'" ],';
	}
