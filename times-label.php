<?php
                              ...
                              $label = str_replace($timeSeries, '<div class=\'clearfix\'>'.$reqfrac1.'<div style="float: left;margin: 0px 10px;">x</div>'.$reqfrac2.'</div>', $label);

                              $timeSeries = array($seq9.' \div'.$seq10, $seq9.' \div '.$seq10, $seq9.'\div '.$seq10, $seq9.'\div'.$seq10);
                              $label = str_replace($timeSeries, '<div class=\'clearfix\'>'.$reqfrac1.'<div style="float: left;margin: 0px 10px;">&divide;</div>'.$reqfrac2.'</div>', $label);

                              $timeSeries = array($seq9.' -'.$seq10, $seq9.' - '.$seq10, $seq9.'- '.$seq10, $seq9.'-'.$seq10);
                              $label = str_replace($timeSeries, '<div class=\'clearfix\'>'.$reqfrac1.'<div style="float: left;margin: 0px 10px;">-</div>'.$reqfrac2.'</div>', $label);

                              $timeSeries = array($seq9.' +'.$seq10, $seq9.' + '.$seq10, $seq9.'+ '.$seq10, $seq9.'+'.$seq10);
                              $label = str_replace($timeSeries, '<div class=\'clearfix\'>'.$reqfrac1.'<div style="float: left;margin: 0px 10px;">+</div>'.$reqfrac2.'</div>', $label);

                              $timeSeries = array($seq9);
                              $label = str_replace($timeSeries, '<div class=\'clearfix\'>'.$reqfrac1.'</div>', $label);

                              $timeSeries = array($seq10);
                              $label = str_replace($timeSeries, '<div class=\'clearfix\'>'.$reqfrac2.'</div>', $label);
